...
describe('markdownToHtml', () => {
  it('renders Markdown body to HTML and preserves the previous body as raw Markdown', async () => {
    const plugin = markdownToHtml();

    const docs = await plugin.transform([createDoc()], createCtx());

    expect(docs[0]).toMatchObject({
      body: expect.stringContaining('<h1>Hello</h1>'),
      bodyType: 'html',
      rawBody: '# Hello',
  ...
  it('treats a missing body type as Markdown', async () => {
    ...
    expect(docs[0].bodyType).toBe('html');
    expect(docs[0].rawBody).toBe('# Hello');
  });
  ...
  it('fails clearly for non-Markdown input', async () => {
    ...
    await expect(
      plugin.transform([createDoc({ body: '<p>Hello</p>', bodyType: 'html' })], createCtx()),
    ).rejects.toThrow('transform:markdown-to-html expects Markdown body input');
  });
});
