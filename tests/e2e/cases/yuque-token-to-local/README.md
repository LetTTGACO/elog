...
这个 case 默认在 `tests/e2e/cases/yuque-token-to-local/elog.config.ts` 的 `e2eProfile.image` 使用本地图床：
...
选择 R2 时，需要提供 `ELOG_E2E_R2_HOST`、`ELOG_E2E_R2_ACCESS_KEY_ID`、`ELOG_E2E_R2_SECRET_ACCESS_KEY`、`ELOG_E2E_R2_BUCKET`、`ELOG_E2E_R2_ENDPOINT`。R2 上传前缀目前在同一个文件的 `imageR2({ prefixKey: 'elog-e2e/yuque-token/' })` 里配置。
