...
Elog 1.0 的飞书云空间来源插件。它遍历“我的空间”或云空间中的指定文件夹，下载其中的
`docx` 文档，并转换为带目录信息的 Markdown `DocDetail`。

## 安装

```bash
pnpm add @elog/plugin-from-feishu-space
```

## 准备飞书应用

使用前需要创建飞书自建应用，取得 `appId` 与 `appSecret`，为应用开通读取云空间、文档和
文档资源所需的权限，并确保应用可以访问目标文件夹。

`folderToken` 是目标文件夹 URL 中的文件夹 Token，也是本插件遍历目录树的起点。
...
export default defineConfig({
  from: fromFeishuSpace({
    ...
    folderToken: process.env.FEISHU_SPACE_FOLDER_TOKEN,
  }),
  plugins: [
    imageLocal({
      outputDir: 'images',
...
| `folderToken` | `string` | —               | 云空间文件夹 Token，必填       |
